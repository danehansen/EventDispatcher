...
describe("EventDispatcher", function() {
  describe("danehansen-EventDispatcher.min.js", function() {
    it("is minified", function() {
      const min = fs.readFileSync(
        path.join(__dirname, "../danehansen-EventDispatcher.min.js"),
        "utf8"
      );
      expect(min.match(/\n/g)).to.be.null;
    });
  ...
  describe("constructor", function() {
    it("sets target to self by default", function() {
      const ed = new EventDispatcher();
      expect(ed._target).to.equal(ed);
      expect(ed._callbacks).to.deep.equal({});
    });

    it("sets target to argument", function() {
      const t = {};
      const ed = new EventDispatcher(t);
      expect(ed._target).to.equal(t);
      expect(ed._callbacks).to.deep.equal({});
    });
  });
